...
def detect_and_smooth_contours(image, min_contour_area=100):
    ...
    # Apply binary thresholding instead of adaptive thresholding
    _, thresh = cv2.threshold(img_blur, 100, 255, cv2.THRESH_BINARY_INV)
    
    edges = cv2.Canny(thresh, 50, 150)
    contours, hierarchy = cv2.findContours(edges, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    # Filtering contours with significant areas and distinguishing inner contours (holes)
    filtered_contours = []
    ...
    # Smooth and offset each filtered contour
...
def compact_nest_contours(contours, canvas_size=(1000, 1000)):
    global nested_contours  # Store transformed contours for DXF export
    nested_image = np.zeros((canvas_size[0], canvas_size[1], 3), dtype=np.uint8)
    nested_contours = []  # Reset nested_contours to store each shifted contour
    current_x, current_y, row_height = 10, 10, 0
    padding = 1  # Set padding between contours

    for contour in contours:
        x, y, w, h = cv2.boundingRect(contour)
        
        # Move to a new row if the contour exceeds canvas width
        if current_x + w + padding > canvas_size[1]:
            current_x = 10
            current_y += row_height + padding
            row_height = 0

        # Add padding to the current contour
        ...
        # Save shifted contour for DXF export
...
# Function to export contours to a DXF file
def export_to_dxf(filename):
    if nested_contours:
        doc = ezdxf.new()
        msp = doc.modelspace()
...
cam = cv2.VideoCapture(1)  # Change to 1 if using an external camera
all_contours = []
processed_contours = []
nested_contours = []  # Store final nested contours for DXF export
